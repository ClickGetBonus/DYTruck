...
static CGFloat const kControlBarHeight = 50;
...
- (UIView *)controlBar
{
    if (_controlBar == nil)
    {
        ...
        UIView *seperator = [[UIView alloc] initWithFrame:CGRectMake(0,
                                                                     kControlBarHeight-1,
                                                                     CGRectGetWidth(self.bounds),
                                                                     1)];
        [seperator setBackgroundColor:[UIColor groupTableViewBackgroundColor]];
        [_controlBar addSubview:seperator];
    }
...
- (UIButton *)confirmButton
{
    if (_confirmButton == nil)
    {
        ...
        [_confirmButton setTitle:@"确认"
                        forState:UIControlStateNormal];
        [_confirmButton setTitleColor:UIColorFromRGB(0xf6a224)
                             forState:UIControlStateNormal];
    ...
    switch (self.dataStructure) {
        ...
        case DataStructureMultiple: {
            ...
            if (index > model.content.count-1) {
                index = 0;
                DLog(@"TLPicker 选择row:%ld, component:%ld 时发生越界", (long)row, (long)component);
            }
            
            self.selectedIndexs[component] = @(index);
            
            for (NSInteger i=component+1; i<self.selectedIndexs.count; i++) {
